...
vector<vector<int>> Load_dataset() {
	...
	//fopen_s(&fn1, "D:\\datasets\\soc-sign-Slashdot090216.txt", "r");
	//fopen_s(&fn1, "D:\\datasets\\p2p-Gnutella06.txt", "r");
	//fopen_s(&fn1, "D:\\datasets\\email-Eu-core-temporal-Dept1.txt", "r");
...
void dfs(pathnode* root) {
	if (root == nullptr)
		return;
	cout << "node: (" << root->row << "," << root->col << ")" << endl;
	if (root->fa != nullptr) {
		cout << "father: (" << root->fa->row << "," << root->fa->col << ")" << endl;
		int r = root->row, c = root->col, fr = root->fa->row, fc = root->fa->col;
		if (r == fr) {
			cout << "col change\ncommon: ";
			for (int i = 0;i < root->common_size;++i) {
				cout << root->col_node_seq[i] << " ";
			}
			cout << endl;
			cout << "diff: ";
			for (int i = 0;i < root->diff_size;++i) {
				cout << root->col_node_seq[i + root->common_size] << " ";
			}
			cout << endl;
		}
		else {
			cout << "row change\ncommon: ";
			for (int i = 0;i < root->common_size;++i) {
				cout << root->row_node_seq[i] << " ";
			}
			cout << endl;
			cout << "diff: ";
			for (int i = 0;i < root->diff_size;++i) {
				cout << root->row_node_seq[i + root->common_size] << " ";
			}
			cout << endl;
		}
	}
...
int main()
{
	...
	vector<vector<int>> I = Load_graph(G);

	cout << "start\n";

	vector<vector<int>> S = RoleSim_Plus2(I, 5, 0.8);
